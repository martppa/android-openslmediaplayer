...
// check parameter
        ...
        {
            static uint64_t pollsctr = 0, pollsanchor = 0;
            if (pollsctr > MAX_POLLS_TO_SKIP)
                pollsctr = 0;
            ...
            for (auto &player : audio_players_info_) {
                if (player.instance->isPollingRequired()) {
                    if (player.instance->isPollSkipping())
                    {
                        player.instance->setVolume(MIN_VOL, MIN_VOL);
                        if (!pollsanchor)
                            pollsanchor = pollsctr;

                        if (pollsctr - pollsanchor >= POLLS_TO_SKIP)
                        {
                            player.instance->stopPollSkipping();
                            pollsanchor = 0;
                            player.instance->setVolume(MAX_VOL, MAX_VOL);
                        }
